...
const allDocumentsSlice = createSlice({
  name: "allDocuments",
  initialState: localStorage.getItem('allDocuments') ? JSON.parse(localStorage.getItem('allDocuments')) : initialState,
  reducers: {
    ...
    deleteExistingDocument: (state, action) => {
      const elementIndex = state.findIndex(element => element.id === action.payload.document.id);
